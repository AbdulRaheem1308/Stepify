...
async function main() {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        const distance = steps * 0.000762; // km
    ...
    console.log(`📅 Seeded 30 days of steps`);
    ...
    const friend = await prisma.user.create({
        data: {
            ...
            name: 'Alice Walker',
            avatarUrl: 'https://api.dicebear.com/7.x/avataaars/png?seed=Aneka',
            wallet: { create: {} },
            streak: { create: {} },
        },
    });
    ...
    // Feed Post from Friend
    await prisma.feedPost.create({
        data: {
            userId: friend.id,
            content: 'Just reached 100k lifetime steps! 🚀',
            type: FeedType.MILESTONE,
            likesCount: 5,
        }
    });
