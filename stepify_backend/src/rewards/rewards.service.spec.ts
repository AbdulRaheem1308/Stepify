...
describe("RewardsService", () => {
    ...
    it("should create and return new wallet if not found", async () => {
      ...
      expect(mockPrisma.wallet.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ userId: "user-1", balance: 0 }),
        }),
      );
    });
  });
  ...
  describe("processStepRewards()", () => {
    it("should award correct points (0.1 per step)", async () => {
      ...
      mockPrisma.user.findUnique = jest
        .fn()
        .mockResolvedValue({ dailyStepGoal: 10000 });
      mockPrisma.transaction.create.mockResolvedValue({});
      ...
      const result = await service.processStepRewards(
        "user-1",
        5000,
        new Date(),
      );
  ...
  describe("redeemReward()", () => {
    it("should throw if reward is not found", async () => {
      ...
      await expect(
        service.redeemReward("user-1", "nonexistent-reward"),
    ...
    it("should throw if user has insufficient coins", async () => {
      ...
      await expect(service.redeemReward("user-1", "r-1")).rejects.toThrow(
        "Insufficient coins",
      );
    });
