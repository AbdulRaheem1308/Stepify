export const getAdminDashboardHtml = () => `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <header>
        ...
        <div class="status-badge">
            ...
            <span id="shield-status">AWAITING API KEY</span>
        </div>
    </header>
    ...
    <script>
        // Prompt for Admin API Key on Load
        let adminApiKey = prompt("Security Lock: Enter Admin API Key to access Secure Endpoints", "");
        
        if (adminApiKey) {
            document.getElementById('shield-status').textContent = "LIVE ATTACK SHIELD ACTIVE";
        }
        ...
        async function fetchMetrics() {
            try {
                ...
                if (res.status === 401 || res.status === 403) {
                    showToast('Unauthorized. Invalid API Key.', 'error');
                    return;
                }
        ...
        document.getElementById('btn-reset-nonces').addEventListener('click', async () => {
            try {
                const res = await fetch('/admin/api/mock/reset-nonces', { 
                    method: 'POST',
                    headers: {
                        'x-admin-api-key': adminApiKey
                    }
