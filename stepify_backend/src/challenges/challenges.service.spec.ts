...
describe('ChallengesService', () => {
  ...
  describe('findUserChallenges', () => {
    it('should return challenges for a user', async () => {
      mockPrismaService.userChallenge.findMany.mockResolvedValue([{ id: 'uc1' }]);
  ...
  describe('create', () => {
    it('should create a challenge', async () => {
      ...
      const res = await service.create({ title: 'T', description: 'D', stepTarget: 100, durationDays: 1, challengeType: 'SOLO', difficulty: 'EASY', imageUrl: '' } as any);
      expect(res.id).toBe('c1');
    });
  });
  ...
  describe('join', () => {
    ...
    it('should throw BadRequest if challenge is not active', async () => {
      mockPrismaService.challenge.findUnique.mockResolvedValue({ id: 'c1', isActive: false });
      await expect(service.join('u1', 'c1')).rejects.toThrow(BadRequestException);
    });
    it('should throw Conflict if already joined', async () => {
      mockPrismaService.challenge.findUnique.mockResolvedValue({ id: 'c1', isActive: true });
      mockPrismaService.userChallenge.findUnique.mockResolvedValue({ id: 'uc1' });
  ...
  describe('updateProgress', () => {
    it('should throw NotFound if user challenge not found', async () => {
      mockPrismaService.userChallenge.findUnique.mockResolvedValue(null);
      await expect(service.updateProgress('u1', 'c1', 100)).rejects.toThrow(NotFoundException);
    });
    it('should throw BadRequest if challenge not ongoing', async () => {
      mockPrismaService.userChallenge.findUnique.mockResolvedValue({ status: 'COMPLETED' });
      await expect(service.updateProgress('u1', 'c1', 100)).rejects.toThrow(BadRequestException);
    });
    it('should update progress successfully', async () => {
      mockPrismaService.userChallenge.findUnique.mockResolvedValue({ status: 'ONGOING', currentSteps: 0, challenge: { stepTarget: 100, rewardCoins: 10 } });
      mockPrismaService.userChallenge.update.mockResolvedValue({ status: 'COMPLETED' });
  ...
  describe('findNewChallenges', () => {
    it('should return challenges not joined by user', async () => {
      mockPrismaService.userChallenge.findMany.mockResolvedValue([{ challengeId: 'c1' }]);
