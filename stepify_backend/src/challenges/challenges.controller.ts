...
import { JoinChallengeDto, UpdateChallengeProgressDto, ChallengeStatus } from './dto/challenge.dto';
...
export class ChallengesController {
     ...
     */
    @Get('my')
    async findMy(@Req() req: AuthenticatedRequest, @Query('status') status?: ChallengeStatus) {
    ...
    async findOngoing(@Req() req: AuthenticatedRequest) {
        return this.challengesService.findUserChallenges(req.user.id, ChallengeStatus.ONGOING);
    }
    ...
    async findCompleted(@Req() req: AuthenticatedRequest) {
        return this.challengesService.findUserChallenges(req.user.id, ChallengeStatus.COMPLETED);
    }
     ...
     */
    @Post('progress')
    async updateProgress(@Req() req: AuthenticatedRequest, @Body() dto: UpdateChallengeProgressDto) {
        return this.challengesService.updateProgress(req.user.id, dto.challengeId, dto.stepsToAdd);
    }
