...
describe("MessagingController", () => {
  ...
  describe("getConversations", () => {
    it("should throw if querying other user", async () => {
      await expect(
        controller.getConversations("u2", { id: "u1" }),
  ...
  describe("getMessages", () => {
    it("should throw if not participant", async () => {
      ...
      await expect(controller.getMessages("c1", { id: "u1" })).rejects.toThrow(
        ForbiddenException,
      );
    });
  ...
  describe("startConversation", () => {
    it("should throw if starting as another user", async () => {
      await expect(
        controller.startConversation(
          { userId: "u2", otherUserId: "u3" } as any,
          { id: "u1" },
        ),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it("should start conversation", async () => {
      mockMessagingService.startConversation.mockResolvedValueOnce({
        id: "c1",
      });
      const res = await controller.startConversation(
        { otherUserId: "u3" } as any,
        { id: "u1" },
      );
  ...
  describe("sendMessage", () => {
    it("should throw if sending as another user", async () => {
      await expect(
        controller.sendMessage({ senderId: "u2" } as any, { id: "u1" }),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it("should throw if not participant", async () => {
      mockMessagingService.isParticipant.mockResolvedValueOnce(false);
      await expect(
        controller.sendMessage({ conversationId: "c1", content: "h" } as any, {
          id: "u1",
        }),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it("should send message", async () => {
      ...
      const res = await controller.sendMessage(
        { conversationId: "c1", content: "hi" } as any,
        { id: "u1" },
      );
