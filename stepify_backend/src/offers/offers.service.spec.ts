...
describe("OffersService", () => {
  ...
  it("should get user offers", async () => {
    ...
    expect(mockPrisma.userOffer.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ userId: "u1", status: "STARTED" }),
      }),
    );
  });
  ...
  describe("completeOffer", () => {
    it("should throw if not started", async () => {
      ...
      await expect(service.completeOffer("u1", "o1")).rejects.toThrow(
        "Offer not found",
      );
    });

    it("should complete offer and reward", async () => {
      mockPrisma.userOffer.findUnique.mockResolvedValueOnce({
        status: "STARTED",
        offer: { rewardCoins: 100 },
      });
