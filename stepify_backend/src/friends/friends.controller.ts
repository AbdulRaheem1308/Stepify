...
export class FriendsController {
     ...
     */
    @Get('leaderboard')
    async getLeaderboard(@CurrentUser() user: any, @Query('type') type?: string, @Query('timeFrame') timeFrame?: string) {
        if (type === 'global') {
            const globalList = await this.friendsService.getGlobalLeaderboard(timeFrame);
            // Mark current user
     ...
     */
    @Post('request')
    async sendRequest(@CurrentUser() user: any, @Body('friendId') friendId: string) {
     ...
     */
    @Post('accept')
    async acceptRequest(@CurrentUser() user: any, @Body('requesterId') requesterId: string) {
     ...
     */
    @Post('boost')
    async sendBoost(@CurrentUser() user: any, @Body('friendId') friendId: string) {
