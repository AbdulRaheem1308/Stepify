...
describe('FriendsService', () => {
  ...
  describe('getFriends', () => {
    ...
    it('should map friends and calculate steps/boosts', async () => {
      ...
      mockPrisma.user.findMany.mockResolvedValueOnce([
        { id: 'f1', name: 'Friend', steps: [{ stepCount: 500 }, { stepCount: 200 }] },
      ]);
      mockPrisma.friendBoost.findMany.mockResolvedValueOnce([{ receiverId: 'f1' }]);
  ...
  describe('getPendingRequests', () => {
    it('should return pending request senders', async () => {
      ...
      mockPrisma.user.findMany.mockResolvedValueOnce([{ id: 'u2', name: 'Sender' }]);
  ...
  describe('searchUsers', () => {
    ...
    it('should search users and map friendship status', async () => {
      ...
      mockPrisma.friendship.findMany.mockResolvedValueOnce([{ userId: 'u1', friendId: 'u2', status: 'PENDING' }]);
  ...
  describe('sendFriendRequest', () => {
    ...
    it('should throw if exists', async () => {
      ...
      await expect(service.sendFriendRequest('u1', 'u2')).rejects.toThrow(ConflictException);
    });
  ...
  describe('acceptFriendRequest', () => {
    it('should throw if not found', async () => {
      mockPrisma.friendship.findFirst.mockResolvedValueOnce(null);
      await expect(service.acceptFriendRequest('u1', 'u2')).rejects.toThrow(NotFoundException);
    });

    it('should accept request', async () => {
      mockPrisma.friendship.findFirst.mockResolvedValueOnce({ id: 'req1' });
      mockPrisma.friendship.update.mockResolvedValueOnce({ status: 'ACCEPTED' });
  ...
  describe('sendBoost', () => {
    it('should throw if boost already sent', async () => {
      ...
      await expect(service.sendBoost('u1', 'u2')).rejects.toThrow(ConflictException);
    });

    it('should throw if not friends', async () => {
      mockPrisma.friendBoost.findFirst.mockResolvedValueOnce(null);
      mockPrisma.friendship.findFirst.mockResolvedValueOnce(null);
      await expect(service.sendBoost('u1', 'u2')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('getMiniLeaderboard', () => {
    it('should sort friends by daily steps and return top 5', async () => {
      mockPrisma.friendship.findMany.mockResolvedValueOnce([{ userId: 'u1', friendId: 'f1', status: 'ACCEPTED' }, { userId: 'u1', friendId: 'f2', status: 'ACCEPTED' }]);
      mockPrisma.user.findMany.mockResolvedValueOnce([
  ...
  describe('removeFriend', () => {
    it('should throw if not found', async () => {
      mockPrisma.friendship.findFirst.mockResolvedValueOnce(null);
      await expect(service.removeFriend('u1', 'u2')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('getGlobalLeaderboard', () => {
    ...
    it('should compute monthly leaderboard', async () => {
      ...
      mockPrisma.wallet.findMany.mockResolvedValueOnce([{ user: { id: 'u1' }, monthlyXp: 100 }]);
    ...
    it('should compute allTime leaderboard', async () => {
      mockRedis.getCache.mockResolvedValueOnce(null);
      mockPrisma.wallet.findMany.mockResolvedValueOnce([{ user: { id: 'u1' }, lifetimePoints: 500 }]);
