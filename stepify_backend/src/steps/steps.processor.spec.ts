...
describe('StepsProcessor', () => {
  ...
  describe('process-sync', () => {
    ...
    it('should successfully process step rewards, update company cache, and log analytics', async () => {
      ...
      prismaService.step.aggregate.mockResolvedValue({ _sum: { stepCount: 5000 } });
      ...
      expect(rewardsService.processStepRewards).toHaveBeenCalledWith('user1', 1000, expect.any(Date));
      ...
      expect(leaderboardGateway.broadcastLeaderboardUpdate).toHaveBeenCalledWith('comp1', [mockMember]);
      expect(postHogService.trackStepSync).toHaveBeenCalledWith('user1', 1000, 'healthkit');
    });

    it('should handle failure in processing step rewards', async () => {
      rewardsService.processStepRewards.mockRejectedValue(new Error('Rewards failed'));
      prismaService.step.aggregate.mockResolvedValue({ _sum: { stepCount: 0 } });
      prismaService.companyMember.findUnique.mockResolvedValue(null);

      await processor.process(mockJob);

      expect(Logger.prototype.error).toHaveBeenCalledWith(expect.stringContaining('Failed to process step rewards'));
    });

    it('should handle corporate leaderboard gracefully if user has no steps sum', async () => {
      rewardsService.processStepRewards.mockResolvedValue(undefined);
      prismaService.step.aggregate.mockResolvedValue({ _sum: { stepCount: null } });
      prismaService.companyMember.findUnique.mockResolvedValue({ userId: 'user1', companyId: 'comp1' });
    ...
    it('should handle failure in corporate leaderboard update', async () => {
      rewardsService.processStepRewards.mockResolvedValue(undefined);
      prismaService.step.aggregate.mockRejectedValue(new Error('Prisma failure'));

      await processor.process(mockJob);

      expect(Logger.prototype.error).toHaveBeenCalledWith(expect.stringContaining('Failed to update corporate leaderboard stats'));
    });

    it('should handle failure in PostHog analytics', async () => {
      rewardsService.processStepRewards.mockResolvedValue(undefined);
      prismaService.step.aggregate.mockResolvedValue({ _sum: { stepCount: 100 } });
      prismaService.companyMember.findUnique.mockResolvedValue(null);
      postHogService.trackStepSync.mockRejectedValue(new Error('Posthog down'));

      await processor.process(mockJob);

      expect(Logger.prototype.warn).toHaveBeenCalledWith(expect.stringContaining('PostHog analytics logging failed'));
    });
  });
  ...
  describe('unknown job', () => {
    it('should log a warning for unknown job type', async () => {
      ...
      await processor.process(mockJob);
      expect(Logger.prototype.warn).toHaveBeenCalledWith(expect.stringContaining('Unknown background job type'));
    });
  });
});
