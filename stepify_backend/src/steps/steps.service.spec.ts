  ...
  describe("syncSteps()", () => {
    it("should throw BadRequestException if no deviceIdentifier provided", async () => {
      await expect(
        service.syncSteps(
          "user-1",
          buildSyncDto({ deviceIdentifier: undefined }),
        ),
    ...
    it("should throw BadRequestException if device not registered", async () => {
      ...
      await expect(service.syncSteps("user-1", buildSyncDto())).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("should throw BadRequestException for negative step count", async () => {
      ...
      await expect(
        service.syncSteps("user-1", buildSyncDto({ stepCount: -1 })),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException for step count exceeding MAX_STEPS_PER_DAY (60000)", async () => {
      mockPrisma.device.findFirst.mockResolvedValue(boundDevice);
      await expect(
        service.syncSteps("user-1", buildSyncDto({ stepCount: 60001 })),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException for jailbroken device", async () => {
      ...
      await expect(
        service.syncSteps(
          "user-1",
          buildSyncDto({
            integrity: { isJailBroken: true, isMockLocation: false },
          }),
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException for mock location device", async () => {
      ...
      await expect(
        service.syncSteps(
          "user-1",
          buildSyncDto({
            integrity: { isJailBroken: false, isMockLocation: true },
          }),
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should reject duplicate nonce (replay attack)", async () => {
      ...
      await expect(
        service.syncSteps("user-1", buildSyncDto({ nonce: "replay-nonce" })),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should reject requests with excessive timestamp drift (> 5 minutes)", async () => {
      ...
      await expect(
        service.syncSteps("user-1", buildSyncDto({ timestamp: oldTimestamp })),
      ).rejects.toThrow(BadRequestException);
    });
