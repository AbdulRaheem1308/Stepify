...
describe("PostHogService", () => {
  ...
  describe("initialization", () => {
    it("should be defined and enabled if API key is present", () => {
      ...
      expect(loggerLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("PostHog analytics enabled"),
      );
    });
    ...
    it("should be disabled if API key is absent", async () => {
      ...
      const emptyService =
        emptyConfigModule.get<PostHogService>(PostHogService);
      expect(loggerWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining("PostHog API key not set"),
      );
  ...
  describe("capture", () => {
    it("should successfully capture an event via fetch", async () => {
      ...
      expect(body.properties.$lib).toBe("wellnex-backend");
    });
  ...
  describe("helpers", () => {
    it("should track trackStepSync", async () => {
      ...
      expect(captureSpy).toHaveBeenCalledWith("u1", "steps_synced", {
        step_count: 1000,
        source: "apple_health",
      });
    });
    ...
    it("should identify user", async () => {
      ...
      expect(captureSpy).toHaveBeenCalledWith("u1", "$identify", {
        $set: { name: "Raheem" },
      });
    });
    ...
    it("should track trackChallengeJoined", async () => {
      ...
      expect(captureSpy).toHaveBeenCalledWith("u1", "challenge_joined", {
        challenge_id: "c1",
        challenge_title: "Summer Step",
      });
    });
    ...
    it("should track trackRewardRedeemed", async () => {
      ...
      expect(captureSpy).toHaveBeenCalledWith("u1", "reward_redeemed", {
        reward_id: "r1",
        coin_cost: 500,
      });
    });
    ...
    it("should track trackAdWatched", async () => {
      ...
      expect(captureSpy).toHaveBeenCalledWith("u1", "ad_watched", {
        ad_type: "video",
        points_earned: 10,
      });
    });
    ...
    it("should track trackUserLogin", async () => {
      ...
      expect(captureSpy).toHaveBeenCalledWith("u1", "user_logged_in", {
        method: "google",
      });
    });
    ...
    it("should track trackAchievementUnlocked", async () => {
      ...
      expect(captureSpy).toHaveBeenCalledWith("u1", "achievement_unlocked", {
        achievement_code: "ACH_1",
        category: "Milestone",
      });
    });
  });
