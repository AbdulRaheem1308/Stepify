...
Quick commands (from `stepify_app/`):
...
```bash
keytool -genkey -v -keystore ~/release_keystore.jks -alias stepify_key -keyalg RSA -keysize 2048 -validity 10000
```
...
```bash
./stepify_app/scripts/build_apk.sh            # release APK (default)
./stepify_app/scripts/build_apk.sh --debug    # debug APK
./stepify_app/scripts/build_apk.sh --aab      # build app bundle (AAB)
./stepify_app/scripts/build_apk.sh --split    # build split per-ABI APKs
```
...
- The workflow builds a universal APK, an AAB, and split per-ABI APKs and uploads them as artifacts (`stepify-apks`, `stepify-aab`).
